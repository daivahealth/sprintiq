...
| **Sprint Health** | `/sprint-health` | `dashboards/sprint-health/active` + `dashboards/sprint-health` | **multi-project default: one card per concurrent active sprint** (each project runs its own lifecycle), ranked worst-pace-first with **cadence-normalized pace** (completion % vs elapsed % of that sprint's own window → on-track/at-risk/behind); click to drill into **five independent panels** — commit activity, daily check-ins, productivity, quality check, release candidates (§4.1.4) |
...
### 4.1.4 Sprint Health detail: five independent panels

Selecting a sprint replaces the drill-down with a header (sprint · project, state, pace) and five panels, in order: **commit activity** (tiles: committers of N assigned, commits with a daily average, PRs raised/open/merged/reviewed, avg time to first review with a >24h-waiting count — scoped to repos linked to the project, over the sprint's own elapsed window; the panel states its own scope judgement — Git has no sprint field, so this is a *window* over mapped repos, not a linkage, and a repo that has never carried a linked PR contributes nothing), **daily check-ins** (a per-developer × per-day grid of Jira status-change counts, paged a week at a time within the sprint's elapsed window; counts whoever made the transition, which is not always the item's assignee, stated on the panel), **productivity** (a per-developer table graded high/medium/low from tickets worked + PRs raised + reviews — never from LOC, which is shown as context only, and the two "highest/lowest LOC contributor" tiles withhold names on purpose: a volume signal, not an attributed leaderboard — see METRICS.md `sprint_productivity_grade` for the tertile rule and the ethics exception it operates under), **quality check** (stories released, rollback share, bugs by priority, and bugs-per-story-released — the ratio and the rollback share both read "not computable" rather than 0.0 when nothing has been released yet), and **release candidates** (one card per Jira fix-version in scope, with delivered-story counts, bugs by priority — via Affects Version, falling back to fix-version with a visible label when Affects Version isn't populated — and a human-recorded planned-release date; test execution is explicitly not shown — pass/fail results live in a test-management tool SprintIQ does not collect, api/README.md §12 #51).

The five panels are deliberately independent, not one query fanned out client-side. `commitActivity` / `productivity` / `qualityCheck` ride on the `sprint-health` response itself (`Promise.all` server-side; any one of the three can resolve `null` for a sprint the underlying computation can't resolve, which the panel renders as a quiet "not available" note rather than a crash). `CheckInGrid` and `ReleaseCandidateList` run their **own** queries and own their own loading/error states, so a slow or failing check-ins or release-candidates fetch degrades that one panel only — the other four stay fully readable. This mirrors the freshness/health-transparency rule in §1: a partial page must say what's missing rather than go blank.
...
Four tiles (commits · developers with a signal `N of M` · PRs opened, `N` merged · committing without assigned work), the **active-developer roster**, the commit timeline, the **12-month trend**, and a **data-health card** carrying both coverage figures — in that order.
...
**The roster is the names behind the tile above it** (added 2026-09-02). `activeDevelopers[]` is the union of commit authors and PR authors in the window — *exactly* the set `totals.developersWithSignal` counts, computed from it, so the list and the count can never disagree about the same window. Each row carries that person's commits, **changed LOC**, PRs opened and PRs merged, and links to their Developer page. Zero is rendered as `0`, never as a blank: a PR-only contributor belongs on the roster and an empty cell reads as missing data.

**Changed LOC per person** (added 2026-09-04) is `additions + deletions`, shown with the `+adds −dels` split beside it — the same figure and the same presentation the Developer page already gives for one person, carried onto the roster so a window can be read without opening it four times. The sum, never a net: a refactor that removes as much as it adds is not almost-no-work. It is accumulated in the pass that already counts commits per developer, so like the other columns it **costs no additional query**. Three rules travel with it, because LOC is the most misread number on the page: the API never orders by it and the UI offers no sort by it (the "Most commits" toggle remains the only ordering a reader can ask for); the provenance note states in words that it is volume and not productivity; and an API that predates the field sends nothing, which renders as **—** rather than `0`, since "changed no lines" and "we cannot tell" are different claims and only one is safe to print next to a person's name.

**Two kinds of entity never appear on it.** Automation, and entities an admin has stated are not developers (DATA-MODEL.md §3.2) — both withheld from every figure that counts PEOPLE while their commits stay in the totals above, so the roster and the tiles keep describing the same window. That split is the point: `copilot-swe-agent` authored 132 commits on the reference tenant and sat here as a person until 2026-09-16. The reverse case is handled by the same table: a developer who also commits from a personal Gmail or under an employee number was two rows, and a stated merge makes them one.
...
**Its bars had never rendered** (fixed 2026-09-04). The row was `flex h-40 items-end`, so its buttons were not stretched and their height came from their content — which is the bar, whose height is a *percentage of the button*. A percentage height against an indefinite parent resolves to `auto`, so every bar was zero pixels tall and the widget drew a 160px row of nothing, in every theme, for as long as it existed. It was consequently read as an unfinished placeholder and came within one instruction of being deleted as one. The row is now `items-stretch` and each button carries `h-full`, so the percentages resolve against a definite height; the button's own `justify-end` still sits the bar on the baseline. **Do not reintroduce `items-end` here** — it silently empties the chart rather than breaking anything a test would catch.

Bars are drawn in **`chart-1` at 80%**, the same token the 12-month chart uses for commits, so one quantity is one colour across the page. The previous `brand` at 30% measured 1.54:1 against the dark surface where 1.0 is indistinguishable from the background; 80% gives 4.29:1 dark and 3.70:1 light, clearing the 3:1 floor for graphical objects. A zero-commit day is `chart-empty` rather than a coloured stub, so no-work reads as no-work instead of as a very small amount of it.
...
**The 12-month trend is the one widget on this page that ignores the window** (added 2026-09-03). Commits or changed LOC per IST calendar month over a fixed year, toggled between the two metrics. Fixed because it is a *trend*: one that resized with the section's selector could not be compared against itself between two visits, so the range selector does not reach it and the subtitle says so outright — that mismatch with the rest of the page is the thing most likely to be misread here, and it sits below the windowed widgets rather than among them.

- **A line, not bars.** The question is the shape of a year — where volume climbed, where it fell away — and twelve separated bars make the reader do that differencing by eye. Bars are right for the daily timeline above, whose columns are discrete days you click into; these twelve points are a trajectory.
- **One metric at a time, on an absolute axis.** The toggle is what buys this: the Developer page's chart has to draw its LOC overlay on a *relative* scale to share an axis with commits, and a relative axis cannot answer "how much". Here the y-axis is labelled in the metric's own units, compacted (`4.5k`, `2.4M`) because LOC and commit counts differ by four orders of magnitude, and always anchored at zero so the same volume does not redraw as a different shape when the metric is switched. The interval is the **smallest** of 1, 2 or 5 × a power of ten that divides the axis into at most six bands — chosen by resulting band count rather than by rounding the range up to a nice number, which collapsed a 20M LOC axis into `0 / 10M / 20M` and stranded every month in one middle band. Searching upwards takes 5M before 10M is ever considered, and guarantees at least four bands at any scale.
- **Months before collection are a break in the line, never a zero.** A month the backfill never walked reports zero commits, and plotting that zero asserts nobody committed — a claim the data does not make. Uncollected months are shaded, carry no marker, and the line is drawn as one path per contiguous collected run so the gap is made of absence rather than painted over a point already on the path. The caption names the floor ("collected back to March 2026"), or says the depth is unestablished when the API reports no watermark at all. Same null-is-not-zero rule as the "committing without assigned work" tile.
- **The running month is drawn provisional, not dropped.** The window always ends with the current IST month, so its last point is routinely a few days of data occupying the same width as thirty. Drawn solid it is indistinguishable from a finished month — on real data, 177 commits on the 3rd sat beside a ~4,000 baseline and the line dived to the floor, reading as a collapse in delivery that had not happened. The final segment is therefore **dashed**, its marker **hollow**, its tooltip says *month to date*, and the provenance note says so in words. This is the same error the backfill floor guards at the other end of the axis (a partial month drawn as a whole one); the difference is that a part-walked month is excluded outright while the running month is kept, because the reader does want the month so far. Which month is in progress is read from the clock in the browser rather than sent by the API — it is a calendar fact, so an older backend still gets the honest rendering.
- **Team totals only.** No per-developer breakdown and no drill-down: a monthly LOC series per person is the vanity ranking CLAUDE.md rules out, and the provenance note repeats that changed LOC is volume, not productivity.
