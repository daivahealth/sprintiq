...
Selecting a sprint replaces the drill-down with a header (sprint · project, state, pace) and five panels, in order: **commit activity** (tiles: committers, commits, PRs raised/reviewed, avg time to first review — scoped to repos linked to the project, over the sprint's own window), **daily check-ins** (a per-developer × per-day grid of Jira status-change counts, paged a week at a time within the sprint's elapsed window), **productivity** (a per-developer table graded high/medium/low from tickets worked + PRs raised + reviews — never from LOC, which is shown as context only, and the two "highest/lowest LOC contributor" tiles withhold names on purpose: a volume signal, not an attributed leaderboard), **quality check** (stories released, rollback share, bugs by priority), and **release candidates** (one card per Jira fix-version in scope, with delivered-story counts, bugs by priority, and a human-recorded planned-release date; test execution is explicitly not shown — pass/fail results live in a test-management tool SprintIQ does not collect).
