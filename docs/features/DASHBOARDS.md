...
Its bars are drawn in **`chart-1` at 80%** — the same token the 12-month chart uses for commits, so one quantity is one colour across the page. They were previously `brand` at 30% alpha, which over the dark surface came out at a contrast ratio of **1.54:1** (1.0 being indistinguishable from the background): the widget rendered a fully populated week and was reported as an empty placeholder awaiting content, and nearly got deleted as one on 2026-09-04. The replacement measures 4.29:1 dark and 3.70:1 light, clearing the 3:1 floor for graphical objects. A zero-commit day is `chart-empty` rather than a coloured stub, so no-work reads as no-work instead of as a very small amount of it.
