...
- **Exclusions (global defaults):** bot/automation accounts excluded from people metrics; merge commits excluded from authorship churn; reverts flagged; draft PRs excluded from review-time until marked ready. Exclusions are configurable per tenant and recorded in `metric_definition`.
  ...
  - **Commit/author-side exclusion is partly implemented** (2026-09-16). Commits carry no bot flag of GitHub's own, so the roll-up falls back to the `name[bot]` convention plus a list of first-party accounts that do not follow it — which now includes `copilot-swe-agent` (Copilot authors commits under a different login than the one it opens PRs under, so it escaped the check entirely and sat in the roster as a person with 132 commits) and `root` (the container user a commit picks up when git has no configured identity). Beyond those global truths, an admin can withhold any entity per tenant via `correlation_identity_override` (DATA-MODEL.md §3.2). In both cases the exclusion is from figures that count PEOPLE only — the commits stay in every repo and LOC total, so no published number moves.
  - **Still open:** merge-commit exclusion from churn, revert flagging, draft-PR handling, and expressing any of it as `metric_definition` rows. Those remain code-level defaults.
...
- **Formula:** `count(issue_status_history rows) grouped by developer, istDateKey(transitioned_at)`, scoped to the sprint's own item keys (not the project) and the requested date range. Paged seven days at a time by default. `developer` is resolved through the same Jira-login/name → canonical-developer bridge `sprint_productivity_grade` uses — falling back to the raw Jira login when no bridge is recorded — so the same person is never shown under two different names between the grid and the productivity table, even though the two count different units (see that entry).
...
- **Notes:** `rolled_back_pct`'s denominator is every item that **entered `done` in the window, released or not** — not `stories_released`, which additionally requires a release and a CURRENT status of `done`. A rollback by definition flips status away from `done`, so basing the denominator on `stories_released` excluded exactly the items the numerator counts, letting the percentage exceed 100%; the wider denominator makes every rollback (which can only happen to an item that was in `done`) a genuine subset of it. It is **null, not 0, when nothing entered `done` in the window** — a fraction over a zero denominator is "not computable," not "zero" — and the rollback count itself is always shown, independent of whether anything was released. `bugs_per_story_released` is separately null when nothing has been released yet, over its own denominator (`stories_released`). `bugs_logged` counts every bug-type item carried by the sprint, not only ones created inside the window.
...
> All metrics in this section default to team/aggregate scope, are RBAC-gated for any individual view, and are framed for support — never ranking — **except the one scoped, documented exception below** (`sprint_productivity_grade`). See [security/AUTH-AND-RBAC.md §5](../security/AUTH-AND-RBAC.md).
...
- **Formula:** `tickets_worked` = distinct sprint items (by the sprint's own item keys, not the project) a developer transitioned in the window. **Not the same population `sprint_daily_check_ins` counts**, despite both reading the same transitions: this counts distinct ticket *keys* per sprint, the check-in grid counts transition *events* per day — a person who moves one ticket through six statuses shows `1` here and `6` there, and both are correct. What the two panels DO share is identity resolution (the same Jira-login/name → canonical-developer bridge), so the same person is never shown under two different names between them. `score = tickets_worked + prs_raised + prs_reviewed`. Bots and deprovisioned accounts (`isBotDeveloper`/`isAnonymizedAccount`) are excluded from this table entirely — their commits still count in `commit_frequency`/`loc_added_deleted` elsewhere, but not here. Grade is cut by **tertiles of `score`, descending, across this sprint's *signal-carrying* contributors** (those with `score > 0`) — top third `high`, bottom third `low`, middle `medium`. A contributor with commits but no ticket/PR/review signal (`score = 0`) stays in the table but is graded `medium` and excluded from the cut, so a zero-score row cannot shift the boundaries or borrow a "high" grade from an undersized distribution.
...
- **Freshness:** `metric_health.data_freshness` reflects the newest contributing event; stale beyond threshold → shown as stale.
  - **Implemented tenant-wide, not yet per metric** (`GET /api/dashboards/freshness`, rendered on every board via the Scope Bar). Per-metric freshness derived from each value's own contributing events arrives with `metric_health`.
  - **Freshness means completeness, not contact.** The reported figure is `collectedThroughAt`: the point in **source** time every change has actually been collected up to, taken as the **oldest** across active connections — the honest bound on a screen that mixes Jira and GitHub facts.
  - **Completeness is a RANGE, and it is judged against the window on screen.** Collection has two ends — `collectedBackTo` (oldest point walked to) and `collectedThroughAt` (newest point collected up to) — and a board showing `[from, now]` is complete iff `collectedBackTo <= from`. A "last 7 days" board over a complete last 7 days is complete, **however far a 12-month historical walk still has to go**, and says nothing. Only a board whose window actually reaches past `collectedBackTo` reports a shortfall, and it names the date history starts. A single global completeness figure got this wrong in the direction that matters: a tenant part-way through a deep backfill could say nothing but "incomplete" for days, on boards whose numbers were entirely correct — and an unnecessary warning is not free, because it teaches people to ignore the real ones.
  - The two sources reach the range from opposite ends, and both cases are covered: GitHub walks newest-first, so recent data is complete almost at once and `collectedBackTo` descends over days; Jira walks oldest-first (`ORDER BY updated ASC`), so `collectedBackTo` is the floor from the first pass and `collectedThroughAt` climbs.
  - **Why not `lastSyncAt`.** "When did we last call the API" is what freshness reported until 2026-08-17, and it overstates coverage exactly when coverage is worst: a connection deep in a backfill reaches GitHub every five minutes and is eighty pages behind, so the board read "Data as of 2 minutes ago" over history it had not collected. `lastSyncAt` is still reported alongside — it is the only thing separating "running and behind" from "stopped" — but it is liveness, not freshness (api/README.md §12 #30).
