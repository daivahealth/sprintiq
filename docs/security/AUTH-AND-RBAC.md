...
- **Secrets:** all source credentials and webhook secrets stored via a secret reference (vault/KMS), never in plaintext columns, never logged; rotation supported. The secret store today is `SecretsService` (BC-0): a per-tenant encrypted table (`connections_tenant_secret`, AES-256-GCM, app-level master key `SECRETS_ENCRYPTION_KEY`) that resolution checks first, falling back to `process.env[ref]` — an admin pasting a real token in `admin/configuration` and an operator setting a deploy-time env var are two entry points to the same reference-based model, not competing ones. Values are write-only from the admin UI (never echoed back by any response) and never appear in audit metadata — only which ref *keys* changed. Swapping in a real vault/KMS later only changes `SecretsService`'s internals, not callers (`resolve(tenantId, ref)`), or the `tenant_configuration` contract below.
- **Tenant configuration:** admin-managed GitHub, Jira, LLM, notification, metric, and security settings are stored in `tenant_configuration`. Plain settings go in JSON `values`; secret material is never stored directly and must be represented as `secretRefs` pointing to the secret store — the ref name is a `values`-adjacent identifier, never the secret itself, regardless of whether the underlying value came from the DB store or an env var.
