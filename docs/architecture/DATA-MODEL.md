...
| `release` | `id`, `tenant_id`, `connection_id`, `project_key`, `name` (Jira fixVersion name), `external_id?` (Jira version id), `released`, `start_at?`, `release_date?`, `archived`, `planned_release_at?`, `planned_set_by_user_id?`, `planned_set_at?` | Jira's project version (fixVersion) — implemented as `planning_release`, unique on `(tenant_id, project_key, name)`. **A Jira version carries exactly one date** (`release_date`, "expected to finish") and Jira overwrites it with the actual ship day the instant `released` flips true, destroying the plan at the exact moment "planned vs actual" becomes worth asking. `planned_release_at` is therefore **user input** — recorded via `PUT /api/dashboards/release-plan` (admin-only, audited, api/README.md §9) alongside who set it and when — and is the only write in an otherwise read-only feature. The source-owned columns (`external_id`, `start_at`, `release_date`, `released`, `archived`) are written exclusively by the `planning.version.upserted` projection; `planned_release_at`/`planned_set_by_user_id`/`planned_set_at` are written exclusively by the mutation above. The two paths write disjoint column sets on purpose, so a poll can never overwrite a plan Jira has no way to restore. `external_id` is null for a release first seen only as a bare fixVersion name on an issue (`Story.releases`), filled in once the version event for it arrives — a row can therefore exist before its dates do. **The `planning.version.upserted` projection still upserts on `(tenant_id, project_key, name)`, not on `external_id`** — re-keying it needs a migration and a uniqueness change, and has not been done. Consequently, renaming a Jira version's fixVersion name creates a NEW `planning_release` row rather than updating the existing one, orphaning the old row's `planned_release_at`/`planned_set_by_user_id`/`planned_set_at` (the recorded plan silently stops being read, since the RC panel resolves releases by current name). |
