...
Correlation only ever runs **once**, at PR-ingestion time — nothing re-triggers it later. A PR ingested before the Jira story it references exists (e.g. the GitHub repo backfilled before the Jira project did) becomes a **permanent** orphan otherwise. `POST /admin/configurations/correlation/reconcile-orphans` re-attempts matching for every unresolved `pull_request` orphan — pure in-DB lookup (title/branch/commit messages against story keys already ingested), no external API calls.
