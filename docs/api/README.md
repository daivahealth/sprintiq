...
- **Backfill completion** is tracked per-connection via `Connection.backfillCompletedAt` — set when a connection's `collection_mode` flips from `backfill` to `poll` (§3). Ordinary config edits (`backfillDays`/`repoFullName`) do **not** reset it — but `POST …/rebackfill` (§9) **does clear it**, deliberately: a connection re-walking history is no longer fully backfilled, and leaving the stamp in place would tell the sweep scheduler it needs no priority exactly when it needs the most. It is re-set when the re-walk completes, so during a re-backfill `summary.backfillInProgress` counts the re-opened connections and returns to 0 when they finish.
